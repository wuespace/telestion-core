...
const contents = fs.readFileSync(path.join(__dirname, '../../super-linter.log')).toString()

const lines = contents.split('\n')
const errors = lines.filter(line => line.startsWith('[ERROR]'))

for (const error of errors) {
  const results = error.match(/\[ERROR\] ([^:]+):(\d+)(:(\d+))?: (.*)/)
  ...
  console.log(`::error file=${file},line=${line}${col ? `,col=${col}` : ''}::${message}`)
}
